...
def test_entry_transition_marks_session_and_preserves_prior_payload() -> None:
    ...
    assert next_state.plugin_id == "gap_continuation"
    ...
    assert next_state.state_hash is not None
...
def test_entry_bindings_match_the_frozen_contract() -> None:
    ...
    decision = evaluation.decision
    assert isinstance(decision, EntryTemplateRequestV1)
    assert evaluation.evaluation_id == context.evaluation_id
    assert evaluation.plugin_id == "gap_continuation"
    ...
    assert decision.entry_reason_codes == ("GAP_CONTINUATION_BULLISH",)
