...
The paper execution worker needs a durable database for its event ledger,
transactional outbox/inbox, idempotency keys, risk reservations, execution
leases, reconciliation state, and final-flatten state. This is therefore a
runtime requirement for a real paper run, but not for the offline fixture and
replay paths.
...
only on `database-internal`. The execution worker is the only application role
on that network. A judge on another architecture can set
...
| `execution_database_url` | `execution` only | Internal Compose DSN for the execution database role |
...
`002_runtime_safety.sql`. The role is a non-superuser with only `CONNECT` to
the application database, schema `USAGE`, and table
`SELECT`/`INSERT`/`UPDATE`/`DELETE` for the runtime ledger. It cannot create
roles, databases, schemas, tables, or temporary tables, and it cannot connect
to PostgreSQL's default databases.
