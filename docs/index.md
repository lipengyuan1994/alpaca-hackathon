...
|---|---|---|---|
| [`index.html`](index.html) | Public “Stable Income Generator” judge website with V13.5 research, SPY/QQQ comparison, system boundaries, and reproduction path | Judges, demo owner, submission reviewers | Public narrative; evidence labels and linked artifacts remain authoritative |
| [`paper-performance.html`](paper-performance.html) | Light-themed Alpaca paper account dashboard with 30-minute account snapshots, one-year P&amp;L history, and the ten most recent system fills | Judges, demo owner, paper-account reviewers | Broker-reported paper evidence; simulated performance, not live-money results |
| [`deployment/PAPER_PERFORMANCE_REFRESH.md`](deployment/PAPER_PERFORMANCE_REFRESH.md) | Cloudflare Durable Object alarms → GitHub Actions → GitHub Pages: schedule, data/credential boundaries, freshness, controls, failure handling, and verification | Paper operator, frontend, platform owner | Read-only performance-publication runbook; separate from paper execution |
...
| [`architecture/ARCHITECTURE_DESIGN.html`](architecture/ARCHITECTURE_DESIGN.html) | Public, responsive physical, logical, data-flow, PostgreSQL, and container architecture page with controls and contract traceability | Judges, architecture reviewers, platform, execution, release | Implementation-aligned visual architecture record |
...
| [`deployment/PAPER_WHEEL_VULTR.md`](deployment/PAPER_WHEEL_VULTR.md) | Automatic GHCR image publishing, protected Vultr deployment approval, restricted SSH delivery, durable state, and single-scheduler cutover | Execution owner, paper operator, and release owner | Always-on paper deployment runbook; never live authority |
...
| [`research/README.md`](research/README.md) | Routes the six symbols and six strategy families into three independently shareable, self-service research packets | Research owners and reviewers | Delegation index; subordinate to the normative research protocol |
| [`research/ALPACA_HISTORICAL_DATA_GUIDE.md`](research/ALPACA_HISTORICAL_DATA_GUIDE.md) | Individual researcher setup, GET-only Alpaca collection, immutable artifacts, and deterministic backtest commands | Research owners | Operational runbook; subordinate to the normative research protocol |
| [`research/ALPACA_DATA_COLLECTOR.md`](research/ALPACA_DATA_COLLECTOR.md) | Read-only collection helper, immutable Alpaca artifacts, and option request files | Research owners and reviewers | Operational reference; subordinate to the normative research protocol |
