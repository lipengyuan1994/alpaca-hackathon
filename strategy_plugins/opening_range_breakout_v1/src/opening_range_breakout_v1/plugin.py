"""Framework adapter for the frozen Group B opening-range-breakout signal.
...
class Plugin:
    ...
    def _decision(self, context: StrategyContextV1, config: StrategyConfigV1):
        ...
        try:
            break_threshold = _decimal_config(config, "break_fraction_threshold", "0.10")
            volume_threshold = _decimal_config(config, "volume_ratio_threshold", "1.25")
            range_floor = _decimal_config(config, "range_floor", "0.000001")
            intent_ttl = _int_config(config, "intent_ttl_seconds", INTENT_TTL_SECONDS)
        except ValueError:
            return NoTradeV1(primary_reason_code="FEATURE_SCHEMA_MISMATCH")
        if break_threshold <= 0 or volume_threshold <= 0 or range_floor <= 0 or intent_ttl <= 0:
            return NoTradeV1(primary_reason_code="FEATURE_SCHEMA_MISMATCH")
        ...
        for symbol in ALLOWED_UNDERLYINGS:
            ...
            result = evaluate_signal(
                underlying=symbol,
                features=local,
                break_fraction_threshold=break_threshold,
        ...
        if not candidates:
            ...
            return NoTradeV1(primary_reason_code="OPENING_RANGE_BREAKOUT_GATE_NOT_MET")
